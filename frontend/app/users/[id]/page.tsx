    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-5">
      ...
      <div className="border border-border rounded-2xl p-6 bg-card flex items-start gap-5">
        ...
        <div className="flex-1 min-w-0 space-y-1">
          ...
          {dp?.location && <p className="text-xs text-muted-foreground">{dp.location}</p>}
          <div className="flex items-center gap-2 flex-wrap pt-1">
            <span className="text-xs bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-400 px-2 py-0.5 rounded-full font-medium">
              Geliştirici
            </span>
            {dp?.is_open_to_work && (
            ...
            {dp?.years_of_experience != null && (
              <span className="text-xs text-muted-foreground">{dp.years_of_experience} yıl deneyim</span>
            )}
          </div>
      ...
      {dp?.bio && (
        <Section title="Hakkında">
          <p className="text-sm text-muted-foreground whitespace-pre-wrap">{dp.bio}</p>
        </Section>
      )}
      ...
      {skills.length > 0 && (
        <Section title="Yetenekler">
          <div className="flex flex-wrap gap-2">
            {skills.map((sk, i) => (
              <span key={i} className="text-xs bg-blue-50 text-blue-700 border border-blue-100 dark:bg-blue-950/30 dark:text-blue-400 dark:border-blue-900 px-2 py-0.5 rounded-full">
                {sk}
              </span>
            ))}
          </div>
            ...
            {workExp.map((exp, i) => (
              <div key={i}>
                <div className="flex justify-between items-baseline gap-2">
                  <span className="text-sm font-semibold text-foreground">{exp.position}</span>
                ...
                {exp.description && (
                  <p className="text-xs text-muted-foreground mt-1 whitespace-pre-wrap leading-relaxed">{exp.description}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <Section title="Eğitim">
          <div className="space-y-4">
            {education.map((edu, i) => (
              <div key={i}>
                <div className="flex justify-between items-baseline gap-2">
                  <span className="text-sm font-semibold text-foreground">{edu.school}</span>
                  <span className="text-xs text-muted-foreground/70 shrink-0">
                ...
                {edu.description && (
                  <p className="text-xs text-muted-foreground mt-1 whitespace-pre-wrap leading-relaxed">{edu.description}</p>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="space-y-4">
            {projects.map((proj, i) => (
              <div key={i}>
                ...
                {proj.technologies && <p className="text-xs text-muted-foreground mt-0.5">{proj.technologies}</p>}
                {proj.url && (
                  <a href={proj.url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 break-all mt-0.5 block">{proj.url}</a>
                )}
                {proj.description && (
                  <p className="text-xs text-muted-foreground mt-1 whitespace-pre-wrap leading-relaxed">{proj.description}</p>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            {langs.map((l, i) => (
              <div key={i} className="flex items-center gap-3">
                <span className="text-sm text-foreground">{l.language}</span>
            ...
            {certs.map((cert, i) => (
              <div key={i}>
                <div className="flex justify-between items-baseline gap-2">
                  ...
                  {cert.year && (
                    <span className="text-xs text-muted-foreground/70 shrink-0">{fmtMonth(cert.year)}</span>
                  )}
                </div>
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-5">
      ...
      <div className="border border-border rounded-2xl p-6 bg-card flex items-start gap-5">
        ...
        <div className="flex-1 min-w-0 space-y-1">
          ...
          {rp?.company_name && <p className="text-xs text-muted-foreground font-medium">{rp.company_name}</p>}
          {rp?.company_location && <p className="text-xs text-muted-foreground">{rp.company_location}</p>}
          <div className="flex items-center gap-2 flex-wrap pt-1">
            <span className="text-xs bg-purple-100 text-purple-700 dark:bg-purple-950/40 dark:text-purple-400 px-2 py-0.5 rounded-full font-medium">
              Recruiter
            </span>
            {rp?.is_hiring && (
              <span className="text-xs bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400 px-2 py-0.5 rounded-full font-medium">
      ...
      {rp?.bio && (
        <Section title="Hakkında">
          <p className="text-sm text-muted-foreground whitespace-pre-wrap">{rp.bio}</p>
        </Section>
      )}
          ...
          <div className="space-y-2">
            {rp?.company_name && (
              <div className="flex gap-3">
                <span className="text-xs text-muted-foreground/70 w-20 shrink-0 pt-0.5">Şirket</span>
                <span className="text-sm text-foreground">{rp.company_name}</span>
              </div>
            )}
            {rp?.company_industry && (
              <div className="flex gap-3">
                <span className="text-xs text-muted-foreground/70 w-20 shrink-0 pt-0.5">Sektör</span>
                <span className="text-sm text-foreground">{rp.company_industry}</span>
              </div>
            )}
            {rp?.company_location && (
              <div className="flex gap-3">
                <span className="text-xs text-muted-foreground/70 w-20 shrink-0 pt-0.5">Konum</span>
                <span className="text-sm text-foreground">{rp.company_location}</span>
              </div>
            )}
            {rp?.company_website && (
              <div className="flex gap-3">
                <span className="text-xs text-muted-foreground/70 w-20 shrink-0 pt-0.5">Website</span>
                <a
